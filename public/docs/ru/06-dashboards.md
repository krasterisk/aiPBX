...
Первый раздел Dashboard показывает **ключевые метрики** за выбранный период. Откройте: меню → **Дашборды** → **Сводный**.

На экране может появиться блок **AI Insights** — короткие выводы по звонкам за период. Клик по insight откроет отфильтрованный [журнал звонков](./09-calls.md).
